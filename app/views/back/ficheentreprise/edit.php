...
<head>
  ...
  <title>Admin – Modifier Fiche | ImpactVenture</title>
...
<div class="wrapper">
  <nav class="main-header navbar navbar-expand navbar-white navbar-light border-bottom">
    <ul class="navbar-nav">
      ...
      <li class="nav-item"><a href="index.php?action=admin_fiche" class="nav-link text-muted">Fiches Entreprises</a></li>
      <li class="nav-item"><span class="nav-link font-weight-bold">/ Modifier</span></li>
    </ul>
  </nav>
  ...
  <div class="content-wrapper" style="background:#F7F8FC">
    <div class="content-header py-3 px-4"><h5 class="mb-0 font-weight-bold" style="font-family:'Space Grotesk',sans-serif;">Modifier : <?= htmlspecialchars($fiche['nom'] ?? '') ?></h5></div>
    <section class="content px-4 pb-4">
      <div class="row justify-content-center"><div class="col-lg-8">
        <div class="mb-3"><a href="index.php?action=admin_fiche" class="btn btn-sm btn-outline-secondary"><i class="fas fa-arrow-left mr-1"></i>Retour</a></div>
        <?php if(!empty($errors)): ?>
        <div class="alert alert-danger"><ul class="mb-0"><?php foreach($errors as $e): ?><li><?= htmlspecialchars($e) ?></li><?php endforeach; ?></ul></div>
        <?php endif; ?>
        <div class="card border-0 shadow-sm rounded-lg">
          <div class="card-header bg-white"><h6 class="font-weight-bold mb-0">Modifier la Fiche Entreprise</h6></div>
          <div class="card-body">
            <form method="POST" action="index.php?action=admin_fiche_update&id=<?= $fiche['id'] ?>">
              <div class="form-group">
                <label class="font-weight-bold">Nom <span class="text-danger">*</span></label>
                <input type="text" name="nom" class="form-control" value="<?= htmlspecialchars($_POST['nom']??$fiche['nom']) ?>">
              </div>
              <div class="form-group">
                <label class="font-weight-bold">Description <span class="text-danger">*</span></label>
                <textarea name="description" class="form-control" rows="4"><?= htmlspecialchars($_POST['description']??$fiche['description']) ?></textarea>
              </div>
              <div class="row">
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <select name="categorie" class="form-control">
                      ...
                      foreach($cats as $v=>$l): ?><option value="<?=$v?>" <?=$cur===$v?'selected':''?>><?=$l?></option><?php endforeach; ?>
                    </select>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <input type="number" name="score_green" class="form-control" min="0" max="100" value="<?= htmlspecialchars($_POST['score_green']??$fiche['score_green']) ?>">
                  </div>
                </div>
              </div>
              <div class="form-group">
                <label class="font-weight-bold">Mots-clés</label>
                <input type="text" name="mots_cles" class="form-control" value="<?= htmlspecialchars($_POST['mots_cles']??$fiche['mots_cles']) ?>" placeholder="ia, solaire, innovation">
              </div>
              <div class="d-flex">
                <a href="index.php?action=admin_fiche" class="btn btn-outline-secondary mr-2"><i class="fas fa-times mr-1"></i>Annuler</a>
