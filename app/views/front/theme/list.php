<?php $msgs=['created'=>'Thème soumis avec succès !','updated'=>'Thème mis à jour.','deleted'=>'Thème supprimé.']; $msg=isset($_GET['msg'])?($msgs[$_GET['msg']]??''):''; ?>
<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8"/>
...
<section class="max-w-7xl mx-auto px-6 py-6">
  <div class="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
    <div class="relative w-full md:w-80">
      ...
      <input type="text" id="searchInput" placeholder="Rechercher un thème…" class="search-input w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm"/>
    </div>
    <div class="flex flex-wrap gap-2">
      <button onclick="filterCat('all',this)" class="filter-btn active px-4 py-1.5 rounded-full text-sm font-semibold border border-gray-200">Tous</button>
      <button onclick="filterCat('tech',this)" class="filter-btn px-4 py-1.5 rounded-full text-sm font-semibold border border-gray-200">Tech & IA</button>
      <button onclick="filterCat('energie renouvelable',this)" class="filter-btn px-4 py-1.5 rounded-full text-sm font-semibold border border-gray-200">Énergie</button>
      <button onclick="filterCat('agriculture durable',this)" class="filter-btn px-4 py-1.5 rounded-full text-sm font-semibold border border-gray-200">AgriTech</button>
      <button onclick="filterCat('digital',this)" class="filter-btn px-4 py-1.5 rounded-full text-sm font-semibold border border-gray-200">Digital</button>
    </div>
  </div>
</section>
...
<section class="max-w-7xl mx-auto px-6 pb-16" id="themeGrid">
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    <?php
    $catMap=[
      'tech'=>['cls'=>'cat-tech','bar'=>'#534AB7','dot'=>251-251*.7],
      'digital'=>['cls'=>'cat-digital','bar'=>'#1D9E75','dot'=>251-251*.55],
      'energie renouvelable'=>['cls'=>'cat-energie','bar'=>'#1D9E75','dot'=>251-251*.9],
      'agriculture durable'=>['cls'=>'cat-agri','bar'=>'#10B981','dot'=>251-251*.85],
      'economie circulaire'=>['cls'=>'cat-eco','bar'=>'#EF9F27','dot'=>251-251*.75],
      'mobilite verte'=>['cls'=>'cat-mob','bar'=>'#EF9F27','dot'=>251-251*.7],
      'entrepreneuriat'=>['cls'=>'cat-entre','bar'=>'#534AB7','dot'=>251-251*.5],
      'autre'=>['cls'=>'cat-autre','bar'=>'#888','dot'=>251-251*.4],
    ];
    foreach ($themes as $i => $t):
      $cm = $catMap[$t['categorie']] ?? $catMap['autre'];
      $offset = 251 - (251 * $t['score_green'] / 100);
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm card-hover fade-up theme-card"
      ...
      <div class="p-5">
        ...
        <h3 class="brand text-base font-bold text-gray-900 mb-1"><?= htmlspecialchars($t['nom']) ?></h3>
        ...
        <div class="flex flex-wrap gap-1.5 mb-3">
          ...
          <span class="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full"><?= htmlspecialchars($tag) ?></span>
          <?php endforeach; ?>
        </div>
        ...
        <div class="flex gap-2 pt-3 border-t border-gray-100">
          <a href="index.php?action=edit&id=<?= $t['id'] ?>"
            ...
            <svg class="w-3 h-3 inline mr-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
            Modifier
          </a>
          <a href="index.php?action=delete&id=<?= $t['id'] ?>"
            ...
            <svg class="w-3 h-3 inline mr-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
            Supprimer
          </a>
        </div>
      </div>
    ...
    <div class="col-span-3 text-center py-16 text-gray-400">
      ...
      <a href="index.php?action=create" class="btn-primary inline-block mt-4 px-5 py-2.5 rounded-xl text-sm font-semibold">
