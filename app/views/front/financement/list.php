...
<div id="applyModal" class="hidden fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[100]">
    <div class="bg-white rounded-3xl p-8 w-full max-w-lg mx-4">
        ...
        <form action="?action=financement_submit" method="POST">
            ...
            <div class="mb-6">
                <label class="block text-gray-600 font-medium mb-2">Investisseur sélectionné</label>
                <p id="modal_investisseur_name" class="text-xl font-semibold text-[#1D9E75]"></p>
            </div>

            <!-- New: Montant souhaité + Converter -->
            <div class="mb-6">
                <label class="block text-gray-600 font-medium mb-2">Montant souhaité (TND) *</label>
                <input type="number" name="montant_demande" id="montantInput"
                       class="w-full border border-gray-300 rounded-2xl p-4 focus:outline-none focus:border-[#1D9E75]"
                       placeholder="Ex: 50000" required>

                <!-- Currency Converter Result -->
                <div id="conversionResult" class="mt-3 text-sm text-gray-600 bg-gray-50 p-3 rounded-2xl min-h-[48px]">
                </div>
            </div>

            <!-- New: Phone Number -->
            <div class="mb-6">
                <label class="block text-gray-600 font-medium mb-2">Votre numéro de téléphone (pour contact)</label>
            ...
            <div class="mb-6">
                ...
                <textarea name="message" rows="5" class="w-full border border-gray-300 rounded-2xl p-5 focus:outline-none focus:border-[#1D9E75]"
                          placeholder="Décrivez brièvement votre projet et pourquoi cet investisseur..." required></textarea>
            </div>
...
<script>
    // Simple conversion rates (TND → EUR / USD) - you can replace with real API later
    const rates = { EUR: 0.31, USD: 0.33 };
    ...
    document.getElementById('montantInput').addEventListener('input', function() {
        ...
        document.getElementById('conversionResult').innerHTML = `
        ≈ <strong>${eur} EUR</strong> | <strong>${usd} USD</strong>
        <span class="text-xs text-gray-400 ml-2">(taux approximatif)</span>
    `;
    });
</script>
</body>
